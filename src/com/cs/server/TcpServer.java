						...
						while (isConnecting) {
							try {
								...
								if (userData.contains("username=")) {
									users.add(new User(
											Long.parseLong(userData.substring(userData.indexOf("=") + 1,
													userData.indexOf("&"))),
									...
									for (User user : users) {
										if (user.getUserId() == Long
												.parseLong(userData.substring(userData.lastIndexOf("=") + 1))) {
											isConnecting = false;
											users.remove(user);
											break;
										}
									...
									for (int i = 0; i < sendTo.length; i++) {
										...
										for (User user : users) {
											if (user.getUserId() == Long.parseLong(sendTo[i].substring(
													sendTo[i].indexOf("userId=") + 7, sendTo[i].indexOf("&")))) {
												System.out.println(user.getSocket());
												DataOutputStream out = new DataOutputStream(
														user.getSocket().getOutputStream());
												for (User temp : users) {
													if (temp.getUserId() == Long.parseLong(
															sendTo[i].substring(sendTo[i].indexOf("senderId=") + 9))) {
														out.writeUTF(temp.getUsername() + ":"
																+ sendTo[i].substring(sendTo[i].indexOf("message=") + 8,
																		sendTo[i].lastIndexOf("&")));
													}
												}
												break;
											}
